...
serve(async (req) => {
  ...
  try {
    ...
    // Parse CSV and filter for NASDAQ stocks
    const lines = listingCsv.split('\n');
    const headers = lines[0].split(',');
    const stocks = [];

    for (let i = 1; i < Math.min(lines.length, 2000); i++) { // Limit to 2000 for performance
      const row = lines[i].split(',');
      if (row.length >= 3 && (row[2]?.includes('NASDAQ') || row[2]?.includes('NYSE'))) {
    ...
    console.log(`Fetched ${stocks.length} stocks from NASDAQ/NYSE`);
    ...
    console.error('Error in alpha-vantage-nasdaq function:', error);
    return new Response(
